...
#   sentra-update-<stamp>.zip   the payload on its own
#   update-sentra.bat                   ONE self-contained file: the same payload
...
# NOTE: apply-update.bat and update-sentra.bat are never packaged. cmd.exe reads a
...
$zip   = Join-Path $desktop "sentra-update-$stamp.zip"
...
$tmp = Join-Path $env:TEMP 'sentra-update-payload.zip'
...
$updater = Join-Path $desktop 'update-sentra.bat'
...
if (Test-Path $updater) {
    try { Remove-Item -LiteralPath $updater -Force -ErrorAction Stop }
        ...
        $updater = Join-Path $desktop "update-sentra-$stamp.bat"
        Write-Host "[NOTE] Desktop\update-sentra.bat is locked by another process."
        Write-Host "       Writing $(Split-Path -Leaf $updater) instead - close whatever is holding"
        Write-Host "       the old file, delete it, then rename this one to update-sentra.bat."
    }
}
